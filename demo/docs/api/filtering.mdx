...
<CodeGroup>
  <CodeBlock label="Where no author connected" active>
...
```
  ...
  <CodeBlock label="Where one author is connected">
...
Just like [combining query arguments](/api/queries#combining-arguments), it is also possible to combine filters.
...
Learn more about [localization](/api/localization).
...
Learn more about [content stages](/api/content-stages).
