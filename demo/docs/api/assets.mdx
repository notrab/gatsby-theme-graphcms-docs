...
The Asset model is included with every project, and can be modified with custom [Field Types](/schema/field-types), and are [localized](/api/localization) by default, but cannot be deleted.
...
These queries work just like regular [queries](/api/queries), and can be [filtered](/api/filtering) by their [fields](/schema/field-types#asset).
...
[GraphQL aliases](/api/queries#graphql-aliases) are great for querying the same asset url with multiple transformations.
...
Learn more about [Mutations](/api/mutations).
